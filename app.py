...
<body>
    ...
    <script>
        ...
        // Функция вызывается при выборе файла
        ...
        // Привязываем функцию к событию выбора файла
        fileInput.onchange = handleFileSelect;
        
        function showConfetti() {
            const colors = ["#ea580c", "#f97316", "#c2410c"];
            for(let i=0;i<30;i++) setTimeout(()=>{ let c=document.createElement("div"); c.style.cssText="position:fixed;top:-10px;left:"+Math.random()*100+"%;width:"+(Math.random()*6+3)+"px;height:"+(Math.random()*6+3)+"px;background:"+colors[Math.floor(Math.random()*colors.length)]+";border-radius:50%;pointer-events:none;z-index:9999;animation:confettiFall "+(Math.random()*2+2)+"s linear forwards"; document.body.appendChild(c); setTimeout(()=>c.remove(),3000); }, i*30);
        }
        ...
        async function uploadFile() {
            ...
            catch(error){ alert("Ошибка: "+error.message); showSkeleton(false); if(progressContainer) progressContainer.style.display="none"; }
        ...
        function showBlock(id) {
            ...
            if(window.innerWidth<=768 && mobileMenu) mobileMenu.style.display="none";
            window.scrollTo({ top: el ? el.offsetTop-20 : 0, behavior:"smooth" });
        }
        ...
        async function askQuestion() {
            ...
            try{
                ...
                const typing = document.querySelector(".typing");
                if(typing) typing.remove();
                chatDiv.innerHTML += "<div class=\"chat-message-bot\"><span><i class=\"fas fa-exclamation-triangle\"></i> Ошибка</span></div>"; 
            }
        }
